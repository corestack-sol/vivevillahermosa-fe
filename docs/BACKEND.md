...
**Borrado — código muerto encontrado en el camino, sin ningún llamador real:** `src/lib/citasRecordatorios.ts` (reemplazado por el poller nuevo), `src/lib/email.ts` (sus 3 funciones de Resend ya no las llamaba nadie — ni alertas, ni citas, ni resolución de solicitudes, todo eso ya lo maneja el backend), `src/lib/openRouterClient.ts` (sin ningún importador desde que IA se migró). `.env.example` se limpió en consecuencia: fuera `RESEND_API_KEY`, `EMAIL_FROM`, `GEMINI_API_KEY`, `OPENROUTER_API_KEY` — `JWT_SECRET` y `CRON_SECRET` se quedan, siguen teniendo un uso real.

**Corregido — este documento tenía varias afirmaciones desactualizadas, encontradas al verificar código en vez de confiar en el propio doc:**
- §3 decía "escritura pendiente" — **llevaba varias fases siendo falso**, `PublishForm.tsx`/`dashboard/propiedades/page.tsx` ya llaman `POST`/`PATCH`/`DELETE /propiedades` reales desde el PR `feat/propiedades-escritura-backend`, anterior incluso a servicios/colonias-tendencia.
- §13 punto 3 decía que quedaban módulos usando `fetch('/api/...')` relativo — ya no queda ninguno, `src/app/api/` está vacío desde antes de hoy.
- §13 punto 5 decía que `esAdmin` no viajaba en `BackendUser` — ya se había corregido, no quedó registrado en ningún changelog.
- §13 punto 9 asumía que `OPENROUTER_API_KEY`/`GEMINI_API_KEY`/`RESEND_API_KEY` seguían siendo obligatorios — ya no los lee ningún código vivo (ver borrado de arriba).
- §14 (backend, `BACKEND.md` de ese repo) tenía sus 6 líneas de "pendiente" completas desde hace tiempo sin que ningún changelog lo registrara — incluidas las cuotas de IA por usuario, que ya existen en cada guard de `src/modules/ia/guards/*-rate-limit.guard.ts` del backend.

**Lección para la próxima auditoría:** varias de estas correcciones no tenían ningún rastro en el propio changelog de este documento — el trabajo se hizo pero nadie volvió a auditar el doc contra el código después. Vale la pena repetir este tipo de pasada (grep de imports reales, no confiar en la prosa) cada vez que se retome el proyecto tras una pausa larga.
...
3. [Propiedades — ✅ completo](#3-propiedades--completo)
...
## 3. Propiedades — ✅ completo

> **Corregido 2026-08-13 (auditoría contra el código real, no contra este documento).** La entrada de 2026-08-10 de abajo decía que la escritura (`POST`/`PATCH`/`DELETE`) seguía simulándose en `localStorage` — **ya no es cierto y no lo era desde hace varias fases** (el PR que migró esto, `feat/propiedades-escritura-backend`, es anterior incluso a servicios/colonias-tendencia). Verificado: `PublishForm.tsx` llama `backendFetch('/propiedades', { method: 'POST' })` y `POST /propiedades/fotos`; `dashboard/propiedades/page.tsx` llama `PATCH`/`DELETE /propiedades/:id` reales. `src/lib/propiedadesLocales.ts`/`estadoOverrides.ts` (§13 punto 11) ya no los usa el flujo de publicar/editar/pausar — quedan otros importadores sueltos sin auditar (no se revisó a fondo en esta pasada, ver §13 punto 11 para el detalle real). Lectura Y escritura de `Property` son 100% reales contra el backend nuevo. Usa el modelo `Property` de §1.
...
3. **✅ Completo (2026-08-13).** Con `src/app/api/**` vacío (punto 1), ya no queda ningún módulo "que siga en Next.js" al que un `fetch('/api/...')` relativo tuviera sentido — confirmado `grep -r "fetch('/api" src/` sin resultados en todo el repo. Todo llama a `backendFetch`/`backendFetchServer` apuntando a `${NEXT_PUBLIC_API_URL}/...`, incluidas las 8 páginas de `/admin/**` y las 5 rutas de `/ia/*`.
...
5. **✅ Completo (2026-08-13) — el gap ya se cerró.** `AuthContext.tsx` llama a `backendFetch('/auth/me')`. El gap de `esAdmin` (registro 2026-08-10) ya no existe: `BackendUser` (`src/lib/backendApi.ts`) incluye `esAdmin: boolean` — verificado directo en el archivo.
...
9. **✅ Completo (2026-08-13), con una excepción esperada.** `OPENROUTER_API_KEY`/`GEMINI_API_KEY` ya no los lee ningún código vivo — `src/lib/openRouterClient.ts` (el único lector) se borró por no tener ningún importador. `RESEND_API_KEY`/`EMAIL_FROM` también se quitaron — `src/lib/email.ts` (Resend) se borró: sus 3 funciones (`sendAlertaEmail`, `sendCitaRecordatorioEmail`, `sendSolicitudRevisionResueltaEmail`) ya no tenían ningún llamador real (`alertas/notificar` nunca se implementó de verdad ni acá ni en el backend, y la resolución de solicitudes/citas ya manda su propio correo desde el backend). **`JWT_SECRET` y `CRON_SECRET` siguen siendo obligatorios a propósito, no es un pendiente** — `proxy.ts` sigue verificando el JWT localmente (punto 4, decisión de secreto compartido), y `CRON_SECRET` ahora lo usa el poller de desarrollo de `src/instrumentation.ts` (§6) para llamar al backend real.
...
11. **Todo lo que hoy simula backend guardando en `localStorage` del navegador se borra por completo** — no se migra, no se conserva como fallback, se reemplaza directo por llamadas reales. Cada archivo ya tiene su propio comentario `⚠️ BACKEND` explicándolo; esta es la lista completa en un solo lugar:
    - **✅ Hecho para el flujo real de publicar/editar/pausar (§3).** `src/lib/propiedadesLocales.ts`/`estadoOverrides.ts` — `PublishForm.tsx` y `dashboard/propiedades/page.tsx` ya llaman `POST`/`PATCH`/`DELETE /propiedades` reales, no estas funciones. **No auditado a fondo en esta pasada (2026-08-13):** ambos archivos siguen teniendo otros importadores sueltos (`src/app/page.tsx`, `src/components/map/MapView.tsx`, `src/lib/colonias.ts`, `src/lib/localStore.ts`, `src/types/property.ts`) — no se confirmó si son dead code residual o si cumplen alguna otra función; revisar antes de borrar los archivos por completo.
    - **✅ Hecho — el recordatorio de citas (§6) ya llama al backend real, no simula nada.** `src/instrumentation.ts` (poller de desarrollo) y `.github/workflows/citas-recordatorios.yml` del repo del backend (cron real de producción) llaman a `POST /citas/recordatorios/procesar`. Antes de 2026-08-13, `src/lib/citasRecordatorios.ts` (ya borrado) procesaba la tabla `Cita` **local** de este repo — huérfana desde que Citas se migró al backend, así que el poller "funcionaba" sin ningún efecto real (nadie recibía recordatorios). `src/lib/email.ts` (Resend) también se borró por el mismo motivo: sin ningún llamador real.
