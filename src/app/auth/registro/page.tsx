...
const PERKS = [
  { icon: <Shield size={15} />, text: 'Favoritos ilimitados' },
  { icon: <Bell size={15} />,   text: 'Alertas por zona y precio' },
  { icon: <MapPin size={15} />, text: 'Contacto directo a propietarios' },
];
      ...
      <div className="w-full max-w-sm lg:max-w-5xl bg-white rounded-2xl shadow-xl overflow-hidden lg:flex">
        ...
        <div className="relative hidden lg:flex flex-col justify-between w-[460px] flex-shrink-0
          ...
          <div className="relative z-10">
            ...
            <div className="bg-white/10 rounded-2xl p-4 border border-white/15">
              ...
              {PERKS.map((p) => (
                <div key={p.text} className="flex items-center gap-3 text-white/80 text-sm py-1.5">
              ...
              <div className="flex items-center gap-3 text-white/80 text-sm py-1.5">
                <div className="w-6 h-6 rounded-lg bg-white/15 flex items-center justify-center flex-shrink-0">
                  <Check size={15} />
                </div>
                100% gratuito
        ...
        <div className="flex-1 flex flex-col justify-center px-6 py-10 lg:px-10">
          <div className="w-full max-w-sm mx-auto">
            <div className="mb-7">
              <h1 className="text-2xl font-heading font-bold text-gray-900">Crea tu cuenta</h1>
              <p className="text-sm text-gray-500 mt-1">Gratis · Sin tarjeta de crédito</p>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div>
                ...
                <input {...register('nombre')} placeholder="Tu nombre completo"
                  autoComplete="name" className={inputCls(!!errors.nombre)} />
            ...
            <p className="text-center text-sm text-gray-500 mt-6">
              ¿Ya tienes cuenta?{' '}
              <Link href={`/auth/login?next=${encodeURIComponent(next)}`} className="text-brand font-bold hover:underline">Iniciar sesión</Link>
            </p>
          </div>
        </div>
