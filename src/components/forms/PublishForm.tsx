...
export function PublishForm() {
  ...
  // Bottom sheet de resumen — solo móvil, pedido explícito 2026-09-17.
  ...
  // Compartida entre el timeline de escritorio (aside fijo) y el resumen
  // de móvil (bottom sheet, pedido explícito 2026-09-17) — mismo
  // contenido, un solo lugar donde mantenerlo.
  const hayProgresoResumen = resumenPasos.some((r) => r !== null);
  function renderPasoItem(i: number) {
    const label = STEPS[i];
    ...
    return (
      <button
        key={label}
        type="button"
        // Solo se puede saltar a un paso ya completado — evita que
        // alguien brinque a "Contacto" sin haber pasado por la
        // validación de los pasos de en medio.
  ...
  return (
    <div className={`relative mx-auto ${step === 2 ? 'max-w-4xl' : 'max-w-2xl'} ${hayProgresoResumen ? 'pb-24 xl:pb-0' : ''}`}>
      ...
      {/* Resumen de móvil — barra fija abajo + bottom sheet, pedido
          ...
          desaparece solo si se limpian todos los datos". `hayProgresoResumen`
          ya sale de los mismos `resumenPasos` de arriba — en cuanto vuelven
          a ser todos `null` (el usuario borró todo), la barra entera
          desaparece sola, sin estado aparte que rastrear. xl:hidden porque
          en escritorio ya está el aside de al lado. */}
      {hayProgresoResumen && (
        <>
          <button
            type="button"
