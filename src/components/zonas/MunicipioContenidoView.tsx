...
/**
 * Contenido editorial ampliado de la página de un municipio — historia,
 * geografía, economía, qué visitar y conexiones, con las fuentes a la vista.
 ...
 */
export function MunicipioContenidoView({ nombre, contenido }: { nombre: string; contenido: MunicipioContenido }) {
  const { datos, secciones, fuentes, consultado } = contenido;
  return (
    <section aria-label={`Conoce ${nombre}`} className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-5">
        {secciones.map((s) => (
          <div key={s.titulo}>
            <h2 className="font-heading font-bold text-gray-800 mb-1.5">{s.titulo}</h2>
